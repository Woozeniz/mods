...
message.channel.send(new MessageEmbed().setColor('0x348f36').setDescription(`${message.author} Sana Ait isim Verilerini Sildim!`))
}
  
if(member) {
let isimler = db.delete(`isimler_${member.user.id}`) || [];
message.react((client.config.onayemoji))
message.channel.send(new MessageEmbed().setColor('0x348f36').setDescription(`${member} Kullanıcısına Ait isim Verilerini Sildim!`))
